...
def reliability_diagram(probs, labels, num_bins=15, threshold=None, title="Reliability Diagram", save_path=None):
    ...
    # Normalize counts for secondary y-axis
    bin_props = bin_counts / np.sum(bin_counts)

    fig, ax1 = plt.subplots(figsize=(6, 6))
    ax1.bar(bin_centers, bin_accs, width=1.0/num_bins, alpha=0.7, color='blue', label='Outputs', edgecolor='black')
    ...
    # Plot secondary y-axis (sample proportions)
    ax2 = ax1.twinx()
    ax2.bar(bin_centers, bin_props, width=1.0/num_bins, alpha=0.3, color='red')
    ax2.set_ylabel("% of Samples", color='blue')
