...
def compute_sharpness(model, dataloader, loss_fn, device, epsilon=1e-3):
    ...
    - loss_fn: FocalLoss 또는 CrossEntropyLoss
    - device: GPU/CPU 설정
    ...
    for inputs, targets in dataloader:
        ...
        for param in model.parameters():
            if param.requires_grad and param.grad is not None:  # 🔹 None 체크 추가
                param.data += epsilon * param.grad.sign()

        with autocast(enabled=False):  # 🔹 AMP 비활성화
