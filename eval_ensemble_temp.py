...
from utils.temperature_scaling_ensemble import EnsembleWithIndividualTS
...
def forward(self, inputs):
    ...
    for model in self.models:
        logits = rein_forward(model, inputs) 
        logits_list.append(logits)

    ensemble_logits = torch.stack(logits_list).mean(dim=0)  
    return self.temperature_scale(ensemble_logits)  # ✅ TS 적용됨
...
def ensemble_evaluate_with_temp(ensemble_model, test_loader, device):
    outputs = []
    targets = []
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for inputs, target in test_loader:
            ...
            ensemble_logits = ensemble_model.forward(inputs)
    ...
    print("=== Evaluation Results with Temperature Scaling ===")
...
def train():
    ...
    ensemble_model = EnsembleWithIndividualTS(models, device)
    ensemble_model.set_temperature(valid_loader)  # 각 모델별 Temperature 찾기
    print("Temperature Scaling applied to individual models.")
    
    ensemble_evaluate_with_temp(ensemble_model, test_loader, device)
