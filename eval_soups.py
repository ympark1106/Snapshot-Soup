...
def greedy_soup(models, model_names, valid_loader, variant, config, args, device):
    ...
    return soup_params, sorted_models[0][0]
...
def train():
    ...
    parser.add_argument('--soup', '-s', type=str, default='greedy')
    ...
    if args.soup == 'greedy':
        ...
        soup_params, model = greedy_soup(models, model_names, valid_loader, variant, config, args, device)
    ...
    model.load_state_dict(soup_params)
    model.eval() 
